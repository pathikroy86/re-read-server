...
app.get("/api/favorites", verifyJwt, async (req, res) => {
    try {
        ...
        const bookIds = favorites
            .filter((favorite) => mongodb_1.ObjectId.isValid(favorite.bookId))
            .map((favorite) => new mongodb_1.ObjectId(favorite.bookId));
        const books = await db
            .collection("books")
            .find({ _id: { $in: bookIds } })
            .toArray();
        const formattedFavorites = favorites
            .map((favorite) => {
            const book = books.find((item) => item._id.toString() === favorite.bookId);
            if (!book) {
                return null;
            }
            return {
...
app.get("/api/blogs", async (_req, res) => {
    try {
        ...
        const formattedBlogs = blogs.map((blog) => ({
            ...
            status: blog.status,
            commentsCount: comments.filter((comment) => comment.blogId === blog._id.toString()).length,
...
app.post("/api/blogs/:id/comments", verifyJwt, async (req, res) => {
    try {
        ...
        const db = await connectDB();
        const blog = await db.collection("blogs").findOne({
            _id: new mongodb_1.ObjectId(blogId),
        });
        if (!blog) {
            return res.status(404).json({
                success: false,
